...
export const InfoCard: React.FC<InfoCardProps> = ({
  ...
  const bgIconColor =
    titleInfo === "Total User"
      ...
      : "bg-[#ffd2d1]";
  const urlTrend =
    trend === "up" ? "/images/Chart/up.svg" : "/images/Chart/down.svg";
    ...
    <div className="sm:flex sm:flex-col justify-center items-center w-[24%] max-sm:w-full bg-white py-[25px] px-[25px] gap-[32px] rounded-xl">
      <div className="inline-flex w-full justify-between">
        <div className="text-left">
          <div className="opacity-70 text-[#202224] text-base font-gilroy-regular text-[17px] font-bold mb-[10px]">
            {titleInfo}
          </div>
          <div className="text-[#202224] text-[28px] font-normal font-gilroy-bold tracking-wide">
            {dataInfo}
          </div>
        </div>
        <div
          className={`h-[60px] w-[60px] flex justify-center items-center ${bgIconColor} rounded-3xl`}
        >
      ...
      <div className="flex flex-row w-full items-start">
        ...
        <p className="text-[#606060] text-[16px] not-italic font-[600] leading-normal">
          <span className={`${textColorTrend}}`}>{percentageChangeInfo}</span>{" "}
          {changeInfo}
        </p>
      </div>
    </div>
